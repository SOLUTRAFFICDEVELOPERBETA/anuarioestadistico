...
const AuthProvider = ({ children }) => {
  ...
  const onLogIn = (email, password) => {
    try {
      fb.auth.signInWithEmailAndPassword(email, password).then(ref => {
        const { uid } = ref.user
  ...
  const passwordReset = async (email) => {
    try {
      await fb.auth.sendPasswordResetEmail(email).then(() => {
        showMessage('Se envío un correo electrónico al correo para restauración de la contraseña')
        router.push('/auth')
      })
    } catch (error) {
      console.log(error)
  ...
  React.useEffect(() => {
    const getUserInfo = () => {
      ...
      if (auth) {
        try {
          fb.db.collection('users').doc(auth).onSnapshot((doc) => {
            const { role } = doc.data()
            if (role === 'admin') {
              dispatch({
                type: GET_USER,
